...
│   ├── import_to_mysql.py         # [活跃] 推荐结果导入 MySQL（Python）
│   ├── import_recommendations.js  # [活跃] 推荐结果导入 MySQL（Node.js）
│   ├── extract_test_subset.py     # [活跃] 从 MySQL 全量导出数据到 CSV
...
注：`run_import_movies.py`、`run_import_ratings.py`、`run_import_tags.py`、`extract_test_subset.py`、`import_to_mysql.py`、`import_recommendations.js` 均已移至 `scripts/import/` 子目录。
...
- **数据库驱动**：PyMySQL
- **用法**：`python scripts/import/run_import_ratings.py`

### 3. `run_import_tags.py`
- **位置**：`scripts/import/run_import_tags.py`
- **用途**：从 MovieLens `tags.csv` 导入标签数据到 MySQL
- **写入表**：`tags`、`movies_tags`、`users_preferred_tags`
- **特性**：分块读取（chunk_size=50000）
- **数据库驱动**：PyMySQL
- **用法**：`python scripts/import/run_import_tags.py`
...
### 4. `import_qdrant.py`
- **位置**：`scripts/import/import_qdrant.py`
...
- **用法**：`python scripts/import/import_qdrant.py`

### 5. `import_qdrant_remaining.py`
- **位置**：`scripts/import/import_qdrant_remaining.py`
- **用途**：补录首次未同步到 Qdrant 的剩余电影
- **判断依据**：MySQL `vector_synced_at IS NULL`
- **用法**：`python scripts/import/import_qdrant_remaining.py`
...
- **数据来源**：`extract_test_subset_test/`（项目根目录）
- **模型输出**：`models/`（`svd_model.pkl`、`user_cf_model.pkl`、`item_cf_model.pkl`）
...
### 10. `import_to_mysql.py` / `import_recommendations.js`
- **位置**：`scripts/import/import_to_mysql.py`（Python 版本） / `scripts/import/import_recommendations.js`（Node.js 版本）
- **用途**：将离线训练的推荐结果 CSV 导入 MySQL
- **导入方式**：`LOAD DATA INFILE`（高性能）或逐行 INSERT（备选）
- **用法**：`python scripts/import/import_to_mysql.py --user-csv users_recommendations.csv --movie-csv movies_similarities.csv`
...
| 脚本 | 删除原因 |
|------|----------|
| `import_movies.py` | 被 `scripts/import/run_import_movies.py` 替代（用 `mysql.connector` 而非 `pymysql`） |
| `import_ratings.py` | 被 `scripts/import/run_import_ratings.py` 替代（`mysql.connector`） |
| `import_tags.py` | 被 `scripts/import/run_import_tags.py` 替代（`mysql.connector`） |
...
| `recommend/extract_test_subset.py` | 使用 `mysql.connector`，功能已整合到 `scripts/import/extract_test_subset.py` |